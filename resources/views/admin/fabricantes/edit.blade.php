...
<head>
    ...
    <title>Editar Fabricante - SAMS</title>
                ...
                <nav class="space-y-2">
                    ...
                    <div class="relative">
                        ...
                        <div id="gestion-submenu" class="hidden pl-4 pr-4 py-2 space-y-1">
                            ...
                            <!-- Fabricantes - Active Link -->
                            <a href="{{ route('fabricantes.complete') }}" class="sidebar-item w-full flex items-center px-4 py-3 text-indigo-600 hover:bg-indigo-50 rounded-lg">
                                <div class="flex items-center space-x-3">
            ...
            <header class="glass-effect shadow-sm border-b border-gray-200">
                <div class="px-6 py-4">
                    <div class="flex items-center justify-between">
                        <div>
                            <h2 class="text-2xl font-bold text-gray-800">Editar Fabricante</h2>
                            <p class="text-gray-600">Modificar información del fabricante</p>
                        </div>
                        
                        <div class="flex items-center space-x-4">
                            <a href="{{ route('fabricantes.complete') }}" class="px-4 py-2 bg-gray-600 text-white rounded-lg hover:bg-gray-700">
            ...
            <main class="flex-1 overflow-y-auto p-6">
                <div class="animate-fade-in">
                    <div class="pw-card bg-white rounded-xl shadow-lg p-8">
                        <form method="POST" action="{{ route('fabricantes.update', $fabricante) }}" class="space-y-6">
                            @csrf
                            @method('PUT')
                            
                            <!-- Información del Fabricante -->
                            <div class="border-b pb-6">
                                <h4 class="text-lg font-medium text-gray-900 mb-4">Información del Fabricante</h4>
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700 mb-2">Nombre del Fabricante *</label>
                                        <input type="text" name="name" value="{{ $fabricante->name }}" required class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                                    </div>
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700 mb-2">Tipo de Fabricante *</label>
                                        <select name="type" required class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                                            ...
                                            <option value="empresa" {{ $fabricante->type == 'empresa' ? 'selected' : '' }}>🏢 Empresa</option>
                                            <option value="persona" {{ $fabricante->type == 'persona' ? 'selected' : '' }}>👤 Persona</option>
                                        </select>
                                    </div>
                                </div>
                                <div class="mt-4">
                                    <label class="block text-sm font-medium text-gray-700 mb-2">Descripción</label>
                                    <textarea name="description" rows="3" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">{{ $fabricante->description ?? '' }}</textarea>
                                </div>
                            </div>
                            ...
                            <div class="border-b pb-6">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div>
                                        ...
                                        <input type="email" name="email" value="{{ $fabricante->email ?? '' }}" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                                    </div>
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700 mb-2">Teléfono</label>
                                        <input type="tel" name="phone" value="{{ $fabricante->phone ?? '' }}" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                                    </div>
                                </div>
                                <div class="mt-4">
                                    <label class="block text-sm font-medium text-gray-700 mb-2">Dirección</label>
                                    <input type="text" name="address" value="{{ $fabricante->address ?? '' }}" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                                </div>
                                <div class="mt-4">
                                    <label class="block text-sm font-medium text-gray-700 mb-2">Información Adicional de Contacto</label>
                                    <textarea name="contact_info" rows="2" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">{{ $fabricante->contact_info ?? '' }}</textarea>
                                </div>
                            </div>
                            ...
                            <div class="pb-6">
                                ...
                                <div class="flex items-center">
                                    <input type="checkbox" name="active" id="active" value="1" {{ $fabricante->active ? 'checked' : '' }} class="mr-2">
                                    <label for="active" class="text-sm font-medium text-gray-700">Fabricante activo</label>
                                </div>
                            </div>

                            <!-- Botones -->
                            <div class="flex justify-end space-x-4">
                                <a href="{{ route('fabricantes.complete') }}" class="px-6 py-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50">
                                    Cancelar
                                </a>

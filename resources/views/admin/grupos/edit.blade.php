                ...
                <nav class="space-y-2">
                    ...
                    <div class="relative">
                        ...
                        <div id="gestion-submenu" class="hidden pl-4 pr-4 py-2 space-y-1">
                            ...
                            <!-- Grupos - Active Link -->
                            <a href="{{ route('grupos.complete') }}" class="sidebar-item w-full flex items-center px-4 py-3 text-indigo-600 hover:bg-indigo-50 rounded-lg">
                                <div class="flex items-center space-x-3">
                ...
                <div class="px-6 py-4">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-gray-600">Modificar información del grupo</p>
                        </div>
                        
                        <div class="flex items-center space-x-4">
                            <a href="{{ route('grupos.complete') }}" class="px-4 py-2 bg-gray-600 text-white rounded-lg hover:bg-gray-700">
                    ...
                    <div class="pw-card bg-white rounded-xl shadow-lg p-8">
                        <form method="POST" action="{{ route('grupos.update', $grupo) }}" class="space-y-6">
                            ...
                            <div class="border-b pb-6">
                                <h4 class="text-lg font-medium text-gray-900 mb-4">Información del Grupo</h4>
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700 mb-2">Nombre del Grupo *</label>
                                        <input type="text" name="name" value="{{ $grupo->name }}" required class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                                    </div>
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700 mb-2">Lider del Grupo *</label>
                                        <select name="leader_id" required class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                                            <option value="">Selecciona un líder...</option>
                                            @foreach($users as $user)
                                            <option value="{{ $user->id }}" {{ $grupo->leader_id == $user->id ? 'selected' : '' }}>{{ $user->name }} {{ $user->last_name ?? '' }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                <div class="mt-4">
                                    ...
                                    <textarea name="description" rows="3" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">{{ $grupo->description ?? '' }}</textarea>
                                </div>
                            </div>
                            ...
                            <div class="pb-6">
                                ...
                                <div class="flex items-center">
                                    <input type="checkbox" name="active" id="active" value="1" {{ $grupo->active ? 'checked' : '' }} class="mr-2">
                                    <label for="active" class="text-sm font-medium text-gray-700">Grupo activo</label>
                                </div>
                            </div>

                            <!-- Botones -->
                            <div class="flex justify-end space-x-4">
                                <a href="{{ route('grupos.complete') }}" class="px-6 py-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50">
                                    Cancelar
                                </a>
